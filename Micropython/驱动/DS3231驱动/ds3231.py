...
class DS3231:
    ...
    def read_inner_temperature(self):
        """Read temperature from DS3231"""
        temp_reg = 0x11
        temp_data = self.i2c.readfrom_mem(self.addr, temp_reg, 2)
        
        # 高字节为整数部分，低字节高两位为小数部分
        msb = temp_data[0]  # 整数部分
        lsb = temp_data[1]  # 小数部分
        ...
        # 计算温度
        fractional_part = (lsb >> 6) * 0.25
        temperature = msb + fractional_part
        return temperature
