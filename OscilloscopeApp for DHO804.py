...
def resource_path(relative_path):#使用PyInstaller打包时,获取资源路径
...
class OscilloscopeApp(QMainWindow):
    ...
    def voltage_formatter(self, x, pos):#y轴坐标格式设置
        if self.voltage_scale >= 1:
            ...
            return f'{x * 1000:.0f}mV'
        else:
            return f'{x * 1e6:.0f}μV'
    ...
    def autoset(self):#示波器自动设置
        if self.running:
            ...
            pass#为FFT处理波形数据,由于要读取示波器内存中的波形,数据量很大,需要分块处理提高效率

    def process_waveform_data(self, raw_data, chunk_size=1000000):
        data_list = []
    ...
    def FFT_software(self):#软件FFT(scipy)
        ...
        data2 = self.process_waveform_data(raw_data, chunk_size=10000000)
        if self.running:
